...
function EstimateDistribution(past_assortments,v,Σ,solver_env=nothing)
...
function GetOptimalAssortment(λ,n,r,Σ,solver_env=nothing)
    K = length(Σ)
    model = _build_model(solver_env)
...
function EvaluateAssortment(S_new,r,past_assortments,v,n,Σ,solver_env=nothing,best_case=true)
