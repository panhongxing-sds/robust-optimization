...
function _build_model(solver_env=nothing)
    if _HAS_GUROBI
        if solver_env === nothing
            model = direct_model(Gurobi.Optimizer())::JuMP.Model
        else
            model = direct_model(Gurobi.Optimizer(solver_env))::JuMP.Model
        end
    elseif _HAS_HIGHS
        model = direct_model(HiGHS.Optimizer())::JuMP.Model
    else
        error("No solver available. Install Gurobi.jl or HiGHS.jl.")
    end
    set_silent(model)
    return model
end

function SolveWorstCase(new_assortment, assortments, products, ρ, v, L, solver_env=nothing)
    M = length(assortments)
    
    # Create the model
    model = _build_model(solver_env)
...
function SolveRobustOptimizationProblem(assortments, products, v, r, solver_env=nothing)
    ...
    for assortment in S_hat
        ...
        obj_val =  SolveWorstCase(assortment, assortments, products, ρ, v, L, solver_env)
...
function EvaluateWorstAndBestCases(new_assortments, assortments, products, v, r, solver_env=nothing)
 	# Construct L
	L = GenerateL(assortments,products)
    ...
    for assortment in new_assortments
        ...
        assortment_to_ro_obj_val[sort(collect(assortment))] = SolveWorstCase(assortment, assortments, products, ρ, v, L, solver_env)
        assortment_to_oo_obj_val[sort(collect(assortment))] = SolveBestCase(assortment, assortments, products, ξ, v, L, solver_env)
...
function SolveBestCase(new_assortment, assortments, products, ξ, v, L, solver_env=nothing)
...
function SolveOptimisticOptimizationProblem(assortments, products, v, r, solver_env=nothing)
    ...
    for assortment in S_hat_optimistic
        ...
        obj_val = SolveBestCase(assortment, assortments, products, ξ, v, L, solver_env)
        #=
        print(assortment, "\n")
        for tuple in L
            print(ρ[tuple], "\t")
